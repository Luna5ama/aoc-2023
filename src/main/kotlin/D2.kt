...
fun main() {
    val input = """Game 1: 10 green, 9 blue, 1 red; 1 red, 7 green; 11 green, 6 blue; 8 blue, 12 green
...
Game 100: 1 red, 14 green; 17 green, 12 red; 3 green, 7 red, 3 blue; 4 green, 13 red, 3 blue; 5 green, 11 red, 5 blue"""
    ...
    fun parseRound(line: String): Round {
        val colors = line.split(", ").associate {
            val color = it.trim().split(" ")
            color[1] to color[0].toInt()
        }
        return Round(
            colors["red"] ?: 0,
            colors["green"] ?: 0,
    ...
    fun parseGame(line: String): Game {
        ...
        val rounds = game[1].split(";").map { parseRound(it) }
    ...
    val part1 = games.asSequence()
        .filter { game ->
            game.rounds.all { round ->
                round.red <= rCount && round.green <= gCount && round.blue <= bCount
            }
        }
        .sumOf { it.id }
