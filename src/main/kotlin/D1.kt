...
nineninekfp49"""
    ...
    val part1 = input.lineSequence()
        .map { str -> str.first { it.isDigit() } to str.last { it.isDigit() } }
        .map { (a, b) -> a.digitToInt() * 10 + b.digitToInt() }
        .sum()
    ...
    fun matchMin(str: String): Int {
        return digitWords.asSequence()
            .map { it.second to str.indexOf(it.first) }
            .filterNot { it.second == -1 }
            .minBy { it.second }
            .first
    }

    fun matchMax(str: String): Int {
        return digitWords.asSequence()
            .map { it.second to str.lastIndexOf(it.first) }
            .filterNot { it.second == -1 }
            .maxBy { it.second }
            .first
    }
    ...
    val part2 = input.lineSequence()
        .map { str -> matchMin(str) to matchMax(str) }
        .map { (a, b) -> a * 10 + b }
        .sum()
