...
............595.........566.............847............456...................................182.........................791........533....."""
    ...
    data class Num(val value: Int, val x: Int, val y: Int)

    val digits = Array(matrices.size) { arrayOfNulls<Num>(matrices[0].length) }

    for ((y, line) in matrices.withIndex()) {
        fun foundAdjacent(it: MatchResult): Boolean {
            if (it.range.first >= 1) {
                if (isSymbol(line[it.range.first - 1])) {
                    return true
                }
                if (y >= 1) {
                    if (isSymbol(matrices[y - 1][it.range.first - 1])) {
                        return true
                    }
                }
                if (y < matrices.lastIndex) {
                    if (isSymbol(matrices[y + 1][it.range.first - 1])) {
                        return true
                    }
                }
            ...
            if (it.range.last < line.length - 1) {
                if (isSymbol(line[it.range.last + 1])) {
                    return true
                }
                if (y >= 1) {
                    if (isSymbol(matrices[y - 1][it.range.last + 1])) {
                        return true
                    }
                }
                if (y < matrices.lastIndex) {
        ...
        regex.findAll(line).forEach {
            if (foundAdjacent(it)) {
                val num = Num(it.value.toInt(), it.range.first, y)
                for (i in it.range) {
                    digits[y][i] = num
                }
                println(it.value)
    ...
    for ((y, line) in matrices.withIndex()) {
        for ((x, c) in line.withIndex()) {
            if (c != '*') continue
            val nearX = (max(0, x - 1)..(min(matrices[0].lastIndex, x + 1)))
            val nearY = (max(0, y - 1)..(min(matrices.lastIndex, y + 1)))
            val near = nearX.flatMap { x -> nearY.map { y -> Pair(x, y) } }

            val results = near.mapNotNull {
            ...
            if (results.size == 2) {
                sum2 += results[0].value * results[1].value
            }
        }
    }
